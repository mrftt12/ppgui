...
def download_circuit(circuit_key, reported_dttm, run_pf=False):
    ...
    try:
        ...
        with Session.builder.configs(connection_parameters).create() as session:
            ...
            cur = conn.cursor()
            cur.execute(
                f"SELECT PRELOAD_ENCODED_CA FROM GRIDMOD_DEV_TD.UC_POC.NMM_F_TOPOLOGICALNODE_ENCODED_CIRCUITS_C_MC11 where CIRCUIT_KEY = '{circuit_key}'"
            )
...
download_circuit(circuit_key="CKT_1043_18094",
                 reported_dttm="2025-09-01 00:00:00.000", run_pf=True)
