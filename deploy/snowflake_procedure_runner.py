...
MULTICONDUCTOR_VERSION='0.0.1.14'
...
class SnowflakeSProcClient:
    ...
    def create_stage(self, session):
        if self.recreate_stage:
            session.sql(f"CREATE OR REPLACE STAGE {self.stage_name}").collect()
        else:
            session.sql(f"CREATE STAGE IF NOT EXISTS {self.stage_name}").collect()
