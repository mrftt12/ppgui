...
def check_ltc_stable_state(mc_net, controlled_trafo_index, voltage_bandwidth, mode, side="lv"):
    """
    ...
    INPUT:
        ...
        **mode** (String) - controller operation mode. Allowed values: "phase" or "gang".
        
    OPTIONAL:    
        **side** (String, default "lv") - Trafo side of the tap control. Allowed values: "lv", "hv". 
        
    """
    if not _is_controller_oscillating(mc_net, controlled_trafo_index):
        controlled_bus_index = mc_net.trafo1ph.loc[controlled_trafo_index].last_valid_index()[0]
        ...
        if mode=="phase":
            ...
            return len(set(tap_pos))==1 and ~np.any(voltage_violations.values & ~tap_saturation.values)
    return True   
       
def add_load_tap_changer_control(mc_net, trafo_top_level_index, mode, vm_lower_pu, vm_upper_pu, detect_oscillation=False):
    """
    ...
    INPUT:
        ...
        **mode** (String) - controller operation mode. Allowed values: "gang", "phase"
        
        **vm_lower_pu** (float) - Lower control voltage limit in pu 
        
        **vm_upper_pu** (float) - Upper control voltage limit in pu 
    
    OPTIONAL
        **detect_oscillation** (boolean, default False) - If False, controller will not converge if oscillating beween two tap settings. 
                                                          If True, controller will converge as soon as oscillation is detected 
                                                          but voltages may be outside allowed bandwidth
...
def _is_controller_oscillating(net, controlled_trafo_index):
    if 'controller' not in net.keys() or net.controller.empty:
        return False
    for c in net.controller['object']:
        if isinstance(c, LoadTapChangerControl):
            return c.oscillating
    return False
...
class McTrafoController(Controller):
    ...
    def _is_trafo_tapside_delta_connected(self, net, trafo_side):
        if trafo_side=='hv':
            side_idx = net[self.element].loc[self.trafo_top_level_index].index.unique(level='bus')[0]
        else: 
            side_idx = net[self.element].loc[self.trafo_top_level_index].index.unique(level='bus')[1]
        delta_conn = any(net.trafo1ph.loc[(self.trafo_top_level_index,side_idx),'to_phase']!=0)
        return delta_conn
...
class LoadTapChangerControl(McTrafoController):
    """
    ...
    INPUT:
        ...
        **trafo_top_level_index** (int) - First-level index of the transformer that is controlled
        
        **mode** (string) - Operation mode. Must be one of ['phase','gang']
    ...
    def __init__(self, net, trafo_top_level_index, mode, vm_lower_pu, vm_upper_pu, detect_oscillation=False, side="lv", trafotype="2W",
        ...
        if mode not in ['phase', 'gang']:
            print("LineDropControl: mode "+str(mode)+" unsupported, using default (gang)")
            self.mode = 'gang'
        else:
            self.mode = mode                
        ...
        if self._is_trafo_tapside_delta_connected(net, side) and mode!='gang':
            logger.warn("LTC control for trafo with top level index "+str(trafo_top_level_index)+": tap side is delta connected but controller set to 'phase' mode.\n"+ 
                        "Hence controller may use asymmetric tap settings, leading to invalid trafo power results.")
    ...
    def initialize_control(self, net):
        ...
        self.tap_pos_prev1 = None
        self.tap_pos_prev2 = None        
        self.oscillating = False        
    ...
    def control_step(self, net):
        ...
        _, _, V_L_pu, _ = self._estimate_IV(net)        
        vm_pu = V_L_pu
        self.tap_pos = self._read_from_trafo1ph(net, self.side, "tap_pos")
        self.tap_pos_prev2 = np.copy(self.tap_pos_prev1)
        self.tap_pos_prev1 = np.copy(self.tap_pos)
        ...
        #print(Iout)
        #voltage angle at the controlled bus
        
        if self.mode == "phase":
            increment = np.where(self.tap_side_coeff * self.tap_sign == 1,
                                 np.where(np.logical_and(vm_pu < self.vm_lower_pu, self.tap_pos > self.tap_min), 
                                          -1,
                                          np.where(np.logical_and(vm_pu > self.vm_upper_pu, self.tap_pos < self.tap_max), 1, 0)),
                                 np.where(np.logical_and(vm_pu < self.vm_lower_pu, self.tap_pos < self.tap_max), 
                                          1,
                                         np.where(np.logical_and(vm_pu > self.vm_upper_pu, self.tap_pos > self.tap_min), -1, 0)))    
        else:          
            increment = np.where(self.tap_side_coeff * self.tap_sign == 1,
                                 np.where( (min(vm_pu) < self.vm_lower_pu and self.tap_pos > self.tap_min), 
                                           -1,
                                           np.where((max(vm_pu) > self.vm_upper_pu and self.tap_pos < self.tap_max), 1, 0)
                                         ),
                                 np.where( (min(vm_pu) < self.vm_lower_pu and self.tap_pos < self.tap_max),
                                            1,
                                           np.where((max(vm_pu) > self.vm_upper_pu and self.tap_pos > self.tap_min), -1, 0)))    
            

        self.tap_pos += increment
        if self.tap_pos_prev2 is not None and all(self.tap_pos != 0) and all(self.tap_pos==self.tap_pos_prev2):
            print("WARNING: LoadTapChangerControl for trafo index "+str(self.trafo_top_level_index)+" was found oscillating.")
    ...
    def is_converged(self, net):
        ...
        if self.mode=="phase":
            ...
            converged = np.all(np.logical_or(reached_limit, self.vm_lower_pu < min(vm_pu) and max(vm_pu) < self.vm_upper_pu))
