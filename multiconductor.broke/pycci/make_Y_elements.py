...
def _make_ytrafo(net):
    ...
    for (tidx, bus, circuit), from_phase, to_phase, dU, Tap, vn_kv, pcc, vcc, p0, i0, Pn in zip(trafo.index, trafo['from_phase'].values, trafo['to_phase'].values, trafo['tap_step_percent'].values, trafo['tap_pos'].values, trafo['vn_kv'].values,
        ...
        # Derive winding rated voltage from bus vn_kv (source of truth)
        # instead of stored trafo1ph vn_kv which may be incorrect
        bus_vn_kv_ll = terminal_vn_kv[bus * NUM_PHASES + from_phase]
        if from_phase == 0 or to_phase == 0:
            # Wye connection (neutral involved): winding voltage = line-neutral
            VRn = bus_vn_kv_ll / np.sqrt(3) * 1e3 * KOLTC
        else:
            # Delta connection: winding voltage = line-line
            VRn = bus_vn_kv_ll * 1e3 * KOLTC
